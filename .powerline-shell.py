...
def add_cwd_segment(powerline, cwd, maxdepth, cwd_only=False):
    #powerline.append(Segment(powerline, ' \\w ', 15, 237))
    home = os.getenv('HOME')
    ...
    if not cwd_only:
        for n in names[:-1]:
            powerline.append(Segment(
                powerline, '%s' % n, Color.PATH_FG, Color.PATH_BG,
...
if __name__ == '__main__':
    ...
    p.append(Segment(p, p.user_prompt[p.shell], 250, 236))
    #p.append(Segment(p, ' \\w ', 15, 237))
